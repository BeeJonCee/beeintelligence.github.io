...
function calculateQuote() {
  ...
  const totalMin = Math.round(
    baseMin + materialCost + laborCost + overheadCost,
  );
  const totalMax = Math.round(
    baseMax + materialCost * 1.5 + laborCost * 1.3 + overheadCost * 1.2,
  );
...
document.getElementById("bookingForm").addEventListener("submit", function (e) {
  ...
  setTimeout(() => {
    showNotification(
      "Thank you for your booking request! We will contact you soon to confirm your consultation.",
      "success",
    );
    this.reset();
    resetSubmitButton(submitButton, originalText);
